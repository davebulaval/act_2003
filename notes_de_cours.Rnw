...
<<eval = TRUE, echo=FALSE, fig=TRUE , height=7, width=8>>=
...
# Modification affichage des graphiques
par(mfrow = c(1,2))
...
fig$plane3d(resul, draw_polygon = TRUE)
...
<<eval = TRUE, echo=FALSE, fig=TRUE, height=4.5,width=5>>=
...
# Affichage xbarre
text(x = xbarre, y = 0, col = "blue", labels = expression(bar(X)), pos = 3, cex = 1)

# Affichage ybarre
text( x = 3, y = ybarre + 2, col = "blue", labels = expression(bar(Y)), pos = 4, cex = 1)
@
...
resul <- lm(y ~x)
plot(x, y, xlab = expression(X[t]), ylab = expression(Y[t]), main = "Échantillon 1")
abline(resul, col="red2", lwd = 1.5)
...
resul2 <- lm(y2 ~ x2)
plot(x2, y2, xlab = expression(X[t]), ylab = expression(Y[t]), main = "Échantillon 2")
...
# Ajout des segments
p <- p + geom_curve(aes(x=0, xend=15, y=8, yend=8), curvature = 0.68, 
                   arrow=arrow(length=unit(0,"cm"), angle=180), linetype=2) + 
        geom_curve(aes(x=0, xend=15, y=5, yend=5), curvature = 0.7, 
                   arrow=arrow(length=unit(0,"cm"), angle=180), linetype=2) 
...
<<fig  = TRUE, echo = FALSE, width=5, height=7>>=
# Visualisation des variables en fct de l'âge
par(mfrow = c(3,2))
plot(Age ~ Survived + Pclass + Sex + Fare + Embarked, data)
@
\bigskip
...
# Graphe des residus
plot(fitL, which=2)
plot(residuals(fitL), main = "Résidus")
@
...
\begin{align*}
l(\utilde{\beta}) &= \displaystyle\sum_{i=1}^n \ln(f_Y(y_i)) \\
&= \displaystyle\sum_{i=1}^n \ln(f_Y(y_i; \mu_i)) \text{ ; où } \mu_i = g^{-1}(X_i \utilde{\beta}) \\
...
<<echo=FALSE, fig = TRUE>>=
#
# Code source tiré des notes de cours de modèle linéaire automne 2014 de Marie-Pier Côté.
#
library(faraway)
data(orings)
## Graphique
plot(damage/6~temp, orings, xlim=c(20,85), ylim = c(0,1),xlab="Temperature", ylab="Prob de bris",pch=19)

## Modèle linéaire
lmod <- lm(damage/6~temp, orings)
abline(lmod,lwd=2)

## GLM avec lien logistique
logitmod <- glm(cbind(damage,6-damage)~temp, family=binomial, orings)
x <- seq(20,85,1)
lines(x,ilogit(coef(logitmod)[1]+coef(logitmod)[2]*x),lty=2,lwd=2, col = "Red")

## GLM avec lien probit
probitmod <- glm(cbind(damage,6-damage)~temp,family=binomial(link=probit), orings)
lines(x,pnorm(coef(probitmod)[1]+coef(probitmod)[2]*x),lty=3,lwd=2, col = "Blue")
legend(20,0.4,c("Linear","Logit","Probit"),lty=c(1:3),lwd=rep(2,3), col = c("Black", "Red", "Blue"))
@
...
&= \displaystyle\sum_{i=1}^n\Bigg\lbrace(y_i - 1) \bigg[\utilde{X}{}_i\beta - \ln(1 + e^{\utilde{X}{}_i\beta})\bigg] - \ln(1 + e^{\utilde{X}{}_i\beta}) \Bigg\rbrace\\
&= \displaystyle\sum_{i=1}^n\Bigg\lbrace(y_i - 1) \utilde{X}{}_i\beta -  y_i\times\ln(1 + e^{\utilde{X}{}_i\beta}) \Bigg\rbrace\\
\end{align*}
\item Vecteur Score
\begin{align*}
