...
%% Graphe modèle quadratique
<<eval = TRUE, echo=FALSE, fig=TRUE, height=3.75,width=6.5>>=
## Les données simulées
beta <- 0.05
n <- 100
...
\begin{moreInfo}{\emph{Astuce calculatrice}}
	La calculatrice TI-30XZ Multiview permet de créé un tableau de donnée et de sortir rapidement et facilement différentes informations sur une régression à partir des données.
	\begin{itemize}
	\item $\overline{X}$ et $\overline{Y}$ ;
	\item $\sum{t=1}^n X_t$, $\sum{t=1}^n X_t^2$, $\sum{t=1}^n Y_t$, $\sum{t=1}^n Y_t^2$ et $\sum{t=1}^n X_tY_t$ ;
