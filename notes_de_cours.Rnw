...
&\overset{\ref{eq:beta0}}{=} \displaystyle\sum_{t=1}^{n}\big( Y_t - (\overline{Y} - \hat{\beta}_1 \overline{X}) - \hat{\beta}_1 X_t \big) \\
...
Variation totale de $Y_t$ & Variation de $\hat{Y}_t$ & Résidu \\
