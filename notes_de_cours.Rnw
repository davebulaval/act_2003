...
\label{ex:multiple}
On cherche à prédire l'âge d'un cerisier noir en fonction de la hauteur du cerisier et la circonférence de son tronc.
...
<<eval = TRUE, echo=FALSE, fig=TRUE , height=5,width=5>>=
library(scatterplot3d)
# Dataset 
dataTrees <- subset(trees, select = c(Girth, Height))
dataTrees$Age <- rgamma(length(dataTrees$Girth),1, 9)

# Le modele
resul <- lm(Age ~ Girth + Height, dataTrees)

# Affichage des points en 3D
fig <- scatterplot3d(dataTrees, type = "h", color = "blue",
                     angle=55, pch = 16, main = "Âge prédit d'un cerisier noir")
...
%% fig 2
<<eval = TRUE, echo=FALSE, fig=TRUE, height=5,width=5>>=
# Même graphe mais angle différent
# Affichage des points en 3D
fig <- scatterplot3d(dataTrees, type = "h", color = "blue",
                     angle=-55, pch = 16, main = "Âge prédit d'un cerisier noir")
...
\begin{align*}
\frac{\pi_i}{1-\pi_i} &= e^{\utilde{X}{}_i\utilde{\beta}} \\
\frac{1-\pi_i}{\pi_i} &= \frac{1}{e^{\utilde{X}{}_i\utilde{\beta}}} \\
\frac{1}{\pi_i} - 1 &= \frac{1}{e^{\utilde{X}{}_i\utilde{\beta}}} \\
\frac{1}{\pi_i} &= 1 + \frac{1}{e^{\utilde{X}{}_i\utilde{\beta}}} \\
\pi_i &= \frac{1}{1 + \frac{1}{e^{\utilde{X}{}_i\utilde{\beta}}}} \\
\end{align*}
...
\color{blue}
\boxed{\color{black}
\pi_i = \frac{e^{\utilde{X}{}_i\utilde{\beta}}}{1 + e^{\utilde{X}{}_i\utilde{\beta}}}
}
\end{equation}
...
\begin{align*}
l(\utilde{\beta}) &= \sum_{i=1}^n \ln\bigg( Pr(Y_i = y_i|\utilde{X}{}_i) \bigg) \\
&= \sum_{i=1}^n \ln\bigg( \pi_i^{y_i} \times (1 - \pi_i)^{1-y_i}) \bigg) \\
&= \sum_{i=1}^n \bigg( y_i \times \ln(\pi_i) + (1 - y_i)\times\ln(1 - \pi_i) \bigg) \\
&= \sum_{i=1}^n \Bigg( y_i \times \ln\bigg(\frac{e^{\utilde{X}{}_i\utilde{\beta}}}{1 + e^{\utilde{X}{}_i\utilde{\beta}}} \bigg) + (1 - y_i)\times\ln\bigg(\frac{1}{1 + e^{\utilde{X}{}_i\utilde{\beta}}}\bigg) \Bigg) \\
&= \sum_{i=1}^n \Bigg( y_i \times \ln(e^{\utilde{X}{}_i\utilde{\beta}}) - y_i\ln(1 + e^{\utilde{X}{}_i\utilde{\beta}}) - (1 - y_i)\times\ln(1 + e^{\utilde{X}{}_i\utilde{\beta}}) \Bigg) \\
&= \sum_{i=1}^n \Bigg( y_i \times\utilde{X}{}_i\utilde{\beta} - \ln(1 + e^{\utilde{X}{}_i\utilde{\beta}}) \Bigg) \\
\end{align*}
...
&= \sum_{i=1}^nX_{i,j}Y_i - \big(X_{i,j}\times \frac{e^{\utilde{X}{}_i\utilde{\beta}}}{1 + e^{\utilde{X}{}_i\utilde{\beta}}}\big) \\
&= \sum_{i=1}^nX_{i,j}\big(Y_i - \frac{e^{\utilde{X}{}_i\utilde{\beta}}}{1 + e^{\utilde{X}{}_i\utilde{\beta}}}\big) \\
&= X_j^\intercal(Y - M(\utilde{X}{}_i\utilde{\beta}))
\end{align*}
...
\begin{bmatrix}
  ...
  \vdots \\
  \frac{e^{\utilde{X}{}_n\utilde{\beta}}}{1 + e^{\utilde{X}{}_n\utilde{\beta}}}\\ 
\end{bmatrix} 
$$
Ainsi, on déduit que
\begin{equation}
...
&= \sum_{i=1}^nX_{i,j}X_{i,k} \times \Bigg(\frac{e^{\utilde{X}{}_i\utilde{\beta}}\bigg(1 + e^{\utilde{X}{}_i\utilde{\beta}}\bigg) - \bigg(e^{\utilde{X}{}_i\utilde{\beta}}\bigg)^2}{\bigg(1 + e^{\utilde{X}{}_i\utilde{\beta}}\bigg)^2}\Bigg) \\
&= \sum_{i=1}^nX_{i,j}X_{i,k} \times \frac{e^{\utilde{X}{}_i\utilde{\beta}}}{\bigg(1 + e^{\utilde{X}{}_i\utilde{\beta}}\bigg)^2} \\
...
&= \sum_{i=1}^n \ln\bigg( y_i\times \utilde{X}{}_i\utilde{\beta} - e^{\utilde{X}{}_i\utilde{\beta}} -\ln(\fact{y_i}) \bigg) \\
\end{align*}
Par conséquent,
$$
S(\utilde{\beta}) = \frac{\partial}{\partial \utilde{\beta}} l(\utilde{\beta}) = ?
...
\begin{align*}
[S(\utilde{\beta})]_j &= \frac{\partial}{\partial \utilde{\beta}_i} l(\utilde{\beta}) \\
&= \sum_{i =1}^n \bigg(Y_iX_{i,j} - X_{i,j} e^{\utilde{X}{}_i\utilde{\beta}} \bigg)\\
&= \sum_{i =1}^n X_{i,j}\bigg(Y_i -  e^{\utilde{X}{}_i\utilde{\beta}} \bigg)\\
...
&= -\frac{\partial}{\partial\beta_k} \bigg( \frac{\partial}{\partial\beta_j} l(\beta)\bigg) \\
&= -\frac{\partial}{\partial\beta_k} [S(\beta)]_j \\
&= \sum_{i=1}^nX_{i,j}X_{i,k} \times e^{\utilde{X}{}_i\utilde{\beta}}\\
&= \sum_{i=1}^nX_{i,j}\times X_{i,k} \times W_i
\end{align*}
où $W_i$ correspond à
$$
e^{\utilde{X}{}_i\utilde{\beta}}
$$
...
\item $l(\utilde{\beta}) = \sum_{i=1}^n\ln\big(Pr(Y_i = y_i|\utilde{X}{}_i)\big)$
...
\end{align*}
\item $l(\utilde{\beta}) = \sum_{i=1}^n\ln(f_{Y_i|\utilde{X}{}_i}(y_i))$
\item $S(\utilde{\beta}) = \frac{\partial}{\partial\utilde{\beta}} l(\utilde{\beta})$
\item $I(\utilde{\beta}) = \frac{\partial^2}{\partial\utilde{\beta^2}} l(\utilde{\beta})$
\item $\hat{\utilde{\beta}}^{(i+1)} = \hat{\utilde{\beta}}^{(i)} + I^{-1}(\hat{\utilde{\beta}}^{(i)})S(\hat{\utilde{\beta}}^{(i)})$
\end{itemize}
...
\item Fonction de log-vraisemblance
\begin{align*}
l(\utilde{\beta}) &= \sum_{i=1}^n\ln\Big(Pr(Y_i|\utilde{X}{}_i)\Big) \\
&= \sum_{i=1}^n\Bigg\lbrace(y_i - 1) \ln(P_i) + \ln(1 - P_i) \Bigg\rbrace\\
&= \sum_{i=1}^n\Bigg\lbrace(y_i - 1) \ln\bigg(\frac{e^{\utilde{X}{}_i\beta}}{1 + e^{\utilde{X}{}_i\beta}}\bigg) + \ln\bigg(\frac{1}{1 + e^{\utilde{X}{}_i\beta}}\bigg) \Bigg\rbrace\\
&= \sum_{i=1}^n\Bigg\lbrace(y_i - 1) \bigg[\utilde{X}{}_i\beta - \ln(1 + e^{\utilde{X}{}_i\beta})\bigg] - \ln(1 + e^{\utilde{X}{}_i\beta}) \Bigg\rbrace\\
&= \sum_{i=1}^n\Bigg\lbrace(y_i - 1) \utilde{X}{}_i\beta -  y_i\times\ln(1 + e^{\utilde{X}{}_i\beta}) \Bigg\rbrace\\
\end{align*}
\item Vecteur Score
\begin{align*}
[S(\hat{\utilde{\beta}})]_j &= \frac{\partial}{\partial\utilde{\beta}} l(\utilde{\beta}) \\
&= \sum_{i=1}^n \Bigg\lbrace  (y_i - 1)X_{i,j} - y_iX_{i,j} \times \frac{e^{\utilde{X}{}_i\beta}}{1 + e^{\utilde{X}{}_i\beta}}\Bigg\rbrace\\
&= ... \\
&= \sum_{i=1}^n \Bigg\lbrace \frac{y_i}{1 + e^{\utilde{X}{}_i\beta}}\Bigg\rbrace \times X_{i,j}\\
\end{align*}
...
&= -\frac{\partial}{\partial\beta_k} \bigg( \frac{\partial}{\partial\beta_j} l(\beta)\bigg) \\ 
&= -\frac{\partial}{\partial\beta_k} [S(\beta)]_j \\
&=  \sum_{i=1}^n\bigg\lbrace y_i \times \frac{e^{\utilde{X}{}_i\beta}}{1 + e^{\utilde{X}{}_i\beta}} \times \frac{1}{1 + e^{\utilde{X}{}_i\beta}} \times X_{i,j} \times X_{i,k} \bigg\rbrace
\end{align*}
On remarque qu'il y a des $y_i$ dans la matrice d'information de Fisher.
\item Matrice F
\begin{align*}
[F(\beta)]_{j,k} &= E_Y\bigg[ [I(\utilde{\beta})]_{j,k}\bigg] \\
&= E_Y\Bigg[ \sum_{i=1}^n\bigg\lbrace y_i \times \frac{e^{\utilde{X}{}_i\beta}}{1 + e^{\utilde{X}{}_i\beta}} \times \frac{1}{1 + e^{\utilde{X}{}_i\beta}} \times X_{i,j} \times X_{i,k} \bigg\rbrace \Bigg]\\
&= \sum_{i=1}^n \bigg\lbrace E[y_i|\utilde{X}{}_i] \times \frac{e^{\utilde{X}{}_i\beta}}{1 + e^{\utilde{X}{}_i\beta}} \times \frac{1}{1 + e^{\utilde{X}{}_i\beta}} \times X_{i,j} \times X_{i,k} \bigg\rbrace\\
&= \sum_{i=1}^n \bigg\lbrace \frac{1}{1-P_i} \times \frac{e^{\utilde{X}{}_i\beta}}{1 + e^{\utilde{X}{}_i\beta}} \times \frac{1}{1 + e^{\utilde{X}{}_i\beta}} \times X_{i,j} \times X_{i,k} \bigg\rbrace\\
&= \sum_{i=1}^n \bigg\lbrace (1 + e^{\utilde{X}{}_i\beta}) \times \frac{e^{\utilde{X}{}_i\beta}}{1 + e^{\utilde{X}{}_i\beta}} \times \frac{1}{1 + e^{\utilde{X}{}_i\beta}} \times X_{i,j} \times X_{i,k} \bigg\rbrace\\
&= \sum_{i=1}^n \bigg\lbrace \frac{e^{\utilde{X}{}_i\beta}}{1 + e^{\utilde{X}{}_i\beta}}  \times X_{i,j} \times X_{i,k} \bigg\rbrace\\
&= \sum_{i=1}^nX_{i,j}\times W_i \times X_{i,k}
\end{align*}
...
# Ajout des segments
p <- p + geom_curve(aes(x=1.5, xend=13.5, y=7, yend=7), curvature = 1.21, size=1.25,
                   arrow=arrow(length=unit(0,"cm"), angle=180), linetype=1, col = "Red") + 
        geom_curve(aes(x=0, xend=15, y=5, yend=5), curvature = 0.5, size=1.25,
                   arrow=arrow(length=unit(0,"cm"), angle=180), linetype=1, col = "Blue") 
# Ajout texte 
p <- p + annotate("text", x=13, y=7.25, parse=TRUE, size=4, col = "Red", 
                  label="X[2] * '=' * 1 * ' (Hommes)'") +
     annotate("text", x=13, y=3, parse=TRUE, size=4, col = "Blue",
           label="X[2] * '=' * 0 * ' (Femmes)'") 


# Ajout des axes
p <- p + ylab("Y") + xlab(expression(X[1])) + coord_cartesian(xlim=c(0, 15), ylim=c(0, 9))
p
@
...
# Ajout des segments
p <- p + geom_curve(aes(x=0, xend=15, y=7, yend=7), curvature = 1.225, size=1.25,
                   arrow=arrow(length=unit(0,"cm"), angle=180), linetype=1) + 
        geom_curve(aes(x=0, xend=15, y=5, yend=5), curvature = 1.225, size=1.25,
                   arrow=arrow(length=unit(0,"cm"), angle=180), linetype=1) 
# Ajout texte 
p <- p + annotate("text", x=7.75, y=1, parse=TRUE, size=4, 
                  label="beta[2]") + 
     annotate("segment", x=7.5, xend=7.5, y=-0.1, yend=1.8,
                                              arrow=arrow(ends="both"))



# Ajout des axes
p <- p + ylab(expression(hat(Y))) + xlab(expression(X[1])) + coord_cartesian(xlim=c(0, 15), ylim=c(0, 9))
p
